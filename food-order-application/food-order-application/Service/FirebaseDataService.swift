...
class FirebaseDataService: NSObject {
    ...
    func getAllOrders(completion: @escaping (Any)->()){
        db.collection("orders").whereField("userEmailAddress", isEqualTo: UserData.emailAddress).addSnapshotListener { querySnapshot, error in
            ...
            for document in querySnapshot!.documents {
                ...
                let order = OrderModel(orderId: orderId, userEmailAddress: userEmailAddress, items: items, total: total, status: status,timestamp: timestamp.dateValue())
                orders.append(order)
            }
